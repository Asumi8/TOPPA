  ...
  describe 'ソート機能' do
    ...
    context 'タスク一覧画面で「実行状況で並び替え」を押した場合' do
      it '「プレゼン用の資料」が一番上部に表示される' do
        ...
        within ".container" do
          sleep(1)
          status_order = all(".rspec").map(&:text)
          sleep(1)
          expect(status_order).to eq ["プレゼン用の資料", "社内発表用の資料"]
        end
      end
    end
    context 'タスク一覧画面で「期限で並び替え」を押した場合' do
      it '「社内発表用の資料」が一番上部に表示される' do
        ...
        within ".container" do
          sleep(1)
          period_order = all(".rspec").map(&:text)
          sleep(1)
          expect(period_order).to eq ["社内発表用の資料", "プレゼン用の資料"]
        end
      end
    end
    context 'タスク一覧画面で「作成日で並び替え」を押した場合' do
      it '「社内発表用の資料」が一番上部に表示される' do
        ...
        within ".container" do
          sleep(1)
          create_order = all(".rspec").map(&:text)
          sleep(1)
          expect(create_order).to eq ["プレゼン用の資料", "社内発表用の資料"]
        end
      end
    end
  end
